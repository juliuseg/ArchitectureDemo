...
namespace FavouritesEd
{
	public class FavouritesTreeView: TreeViewWithTreeModel<FavouritesTreeElement>
	{
		...
		// Unity used to expose the folder icon name via the internal
		// UnityEditorInternal.EditorResourcesUtility.folderIconName property, which was
		// removed in newer editors. The icon is now just a named built-in resource.
		private static string FolderIconName()
		{
			return EditorGUIUtility.isProSkin ? "d_Folder Icon" : "Folder Icon";
		}
